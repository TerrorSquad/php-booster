...
You can disable, override, or add custom tools via `.git-hooks.config.json`. You have full control over every aspect of a tool's configuration.

**Override an existing tool:**
...
| `command` | `string` | The command to run (e.g., `eslint`, `vendor/bin/phpstan`). |
| `args` | `string[]` | Arguments passed to the command. |
| `type` | `node` \| `php` \| `system` | Where to find the binary (`node_modules/.bin`, `vendor/bin`, or system PATH). |
| `extensions` | `string[]` | Only run on files with these extensions. |
| `includePatterns` | `string[]` | Only run on files matching these glob patterns (e.g., `apps/*/src/*.json`). |
| `passFiles` | `boolean` | Whether to pass the list of staged files to the command (default: `true`). |
| `runForEachFile` | `boolean` | Run the command separately for each file (default: `false`). |
| `stagesFilesAfter` | `boolean` | Re-stage files after running (useful for auto-fixers). |
| `onFailure` | `continue` \| `stop` | `stop` to halt execution immediately on failure (default: `continue`). |
| `group` | `format` \| `lint` \| `analysis` \| `security` \| `refactor` | The tool group for selective execution. |

::callout{type="info"}
**Wildcards vs. Lists:**
Tools like `ESLint`, `Prettier`, and `Rector` automatically handle lists of files. For tools that run on the whole project (like `Knip` or `Composer Validate`), set `"passFiles": false`.
::
...
| Group | Tools |
|-------|-------|
| `lint` | ESLint, Stylelint, PHP Syntax Check, TypeScript, Markdownlint, Composer Validate |
| `format` | Prettier, ECS, Flatten JSON |
| `analysis` | PHPStan, Psalm, Deptrac, jscpd, Knip |
| `security` | PHP Security Checker |
| `refactor` | Rector |
...
| PHP Security Checker | `SKIP_PHP_SECURITY_CHECKER` |
| Composer Validate | `SKIP_COMPOSER_VALIDATE` |
| Markdownlint | `SKIP_MARKDOWNLINT` |
| jscpd | `SKIP_JSCPD` |
| Knip | `SKIP_KNIP` |
