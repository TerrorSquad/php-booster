...
To see detailed explanations of all issues, add the verbose flag:

```bash
composer phpinsights -- -v
```

Or with DDEV:

```bash
ddev composer phpinsights -- -v
```

### Additional Options

You can pass additional PHP Insights options through the composer script:

```bash
# Specify a minimum quality threshold
composer phpinsights -- --min-quality=90

# Analyze without colors (useful for CI)
composer phpinsights -- --no-ansi
