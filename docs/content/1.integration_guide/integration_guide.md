...
PHP Booster no longer bundles a Git hook system. For commit message linting, branch name validation, pre-commit checks, and other Git hook automation, use **[Forge](https://terrorsquad.github.io/forge/)** — a standalone, zero-dependency Git hook manager designed to work alongside this booster.

## JavaScript/TypeScript Projects

For JavaScript or TypeScript projects without PHP, use the **JS/TS only mode** (`-J`):
...
```bash [Terminal]
# Update configs and deps together
curl -sSL https://raw.githubusercontent.com/TerrorSquad/php-booster/main/booster/integrate_booster.sh | bash -s -- --update-configs --update-deps
```
...
| `-J` | **JavaScript/TypeScript only** mode (no PHP tools). |
