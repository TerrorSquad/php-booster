...
class TestOrchestrator:
    ...
    def _test_interactive_mode_standalone(self):
        ...
        try:
            result = subprocess.run(
                ["bash", str(booster_script), "-I", "-v"],  # Added -v for verbose output
                input=input_string,
                text=True,
                capture_output=True,
                cwd=test_dir,
            ...
            if branch_config_path.exists():
                with open(branch_config_path, "r") as f:
                    ...
                    if "PRJ-" not in content:
                        self.log.error(f"Ticket prefix 'PRJ' not found in branch validation config!")
                        return False
                self.log.success("Branch validation config contains correct ticket prefix.")
            
            if result.returncode == 0:
                self.log.success("Standalone interactive test completed successfully!")
                return True
            else:
                self.log.error(f"Standalone interactive test failed with exit code: {result.returncode}")
                return False
    ...
    def _test_interactive_mode_project(self, interactive=True):
        ...
        if not self.state.is_project_created():
            ...
            self.log.info(f"  python test-integration.py setup {self.config.project_type} {self.config.project_name}")
            return False
    ...
    def run_action(self, action: str):
        ...
        actions = {
            ...
            "clean-interactive-test": lambda: (
                self._test_interactive_mode("clean"),
            ),
